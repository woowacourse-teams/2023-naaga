...
class AuthInterceptor() : Interceptor {
    ...
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        if (response.code == 401) {
            ...
            if (failureDto.code == 102) {
                return reRequest(chain).also { if (it != null) response.close() } ?: return response
            } else {
                startLoginActivity()
                Log.e("[ERROR] Auth Interceptor", "$failureDto")
            }
        }
        return response
    }
    ...
    private fun getAccessToken(): String {
        return requireNotNull(NaagaApplication.authDataSource.getAccessToken()) { NO_ACCESS_TOKEN }
    }
